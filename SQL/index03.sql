...
select e.name, e.age from departments d join employees e on e.department_id = d.id where e.age<30 and d.name='Engineering';
-- 6. 직원 테이블에서 직원 수를 계산합니다.
select count(*) from employees;
-- 7. 직원 테이블에서 각 부서의 직원 수를 계산합니다.
select d.name, count(e.department_id) from departments d join employees e 
where e.department_id=d.id group by e.department_id;
-- 8. 직원 평균 나이를 계산합니다.
select avg(age) 평균나이 from employees;
-- 9. 부서별 평균 나이를 계산합니다.
select d.name, avg(e.age) from departments d join employees e where e.department_id=d.id group by e.department_id;
